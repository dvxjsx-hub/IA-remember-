...
function scoreAnswer(challenge, answer) {
  const given = normalizeAnswer(answer);
  const accepted = challenge.accepted.map(normalizeAnswer).filter(Boolean);
  return accepted.some(x => x === given || (x.length >= 5 && given.length >= 5 && (x.includes(given) || given.includes(x))));
}
...
async function answer(gameId, answer) {
  cleanup(); const game = games.get(gameId); if (!game) throw Object.assign(new Error("La partida expiró. Inicia una nueva."), { status: 404 });
  const given = normalizeAnswer(answer); if (!given) throw Object.assign(new Error("Escribe una respuesta."), { status: 400 });
  const correct = scoreAnswer(game.challenge, answer); const points = correct ? (game.challenge.type === "date" ? 15 : 10) : 0;
  game.score += points; game.streak = correct ? game.streak + 1 : 0;
