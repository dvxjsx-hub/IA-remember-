...
function normalize(value = "") {
  return String(value).toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9ñü/]+/gi, " ").replace(/\s+/g, " ").trim();
}
...
// El orden de parseo (archivos ordenados alfabéticamente, línea por línea)
// tiene que ser IDÉNTICO al de scripts/build-memory-index.js: los chunks
// del índice semántico referencian mensajes por posición (start/end) sobre
// este mismo array, sin ningún lookup intermedio.
function loadChatMemory() {
  if (!fs.existsSync(config.CHAT_DIR)) return [];
  const files = fs.readdirSync(config.CHAT_DIR).filter(n => /^Aniveersary.*\.txt$/i.test(n)).sort();
  const messages = [];
  for (const file of files) for (const line of fs.readFileSync(path.join(config.CHAT_DIR, file), "utf8").split(/\r?\n/)) {
    ...
    const speaker = m[2].toUpperCase(); messages.push({ raw: line, date: m[1].trim(), speaker, persona: speaker === "R" ? "river" : "mavc", content: m[3].trim(), normalized: normalize(m[3]), tokens: tokens(m[3]).map(stem) });
  }
  return messages;
}
...
function dateTerms(query) {
  const q = normalize(query), out = [];
  for (const m of q.matchAll(/\b(\d{1,2})\s*[/ -]\s*(\d{1,2})\s*[/ -]\s*(\d{2,4})\b/g)) out.push({ day: m[1].padStart(2, "0"), month: m[2].padStart(2, "0"), year: m[3].length === 2 ? `20${m[3]}` : m[3] });
  return out;
}
function lexicalScore(item, query) {
  const q = normalize(query), terms = tokens(query).map(stem), set = new Set(item.tokens);
  let score = 0, hits = 0;
  if (q.length > 5 && item.normalized.includes(q)) score += 50;
  for (const t of terms) { if (set.has(t)) { score += 10; hits++; } else if (item.normalized.includes(t)) { score += 5; hits++; } }
  if (hits >= 2) score += hits * 5;
  for (const d of dateTerms(query)) { const dt = normalize(item.date); if (dt.includes(`${d.day}/${d.month}/${d.year}`) || dt.includes(`${d.day}-${d.month}-${d.year}`)) score += 80; }
  return score;
}
function truncateMemory(rows) {
  const out = []; let chars = 0;
  for (const m of rows) {
    if (out.length >= MAX_MEMORY_MESSAGES) break;
    const room = MAX_MEMORY_CHARS - chars;
    if (room <= 80) break;
    const content = m.content.length > room ? m.content.slice(0, room) + "…" : m.content;
    out.push({ ...m, content }); chars += content.length + 32;
  }
  return out;
}
...
async function searchMemory(query) {
  ...
  // Cada chunk semántico referencia un rango [start,end] directo en CHAT_MEMORY.
...
function stats() {
  return { messageCount: CHAT_MEMORY.length, ...embeddingsService.stats() };
}
