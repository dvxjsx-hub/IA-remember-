...
const STOPWORDS = new Set("a al algo aqui asi aun aunque con como cual cuando de del desde donde dos el ella ellas ellos en era es esa esas ese estos esta estas este fue ha hay hasta la las lo los me mi mis muy ni no nos o para pero por que quien se si sin sobre su sus te un una uno unos y ya yo recuerda recuerdas acuerdas acuerdo vez dia historia historial chat hablamos hablar habiamos habían habían".split(" "));
...
function loadChatMemory() {
  ...
  for (const file of files) {
    let currentDate = "";
    for (const line of fs.readFileSync(path.join(config.CHAT_DIR, file), "utf8").split(/\r?\n/)) {
      const heading = line.match(/^##\s+(\d{1,2}\/\d{1,2}\/\d{2,4})\s*$/);
      if (heading) { currentDate = heading[1]; continue; }
      const item = line.match(/^\s*-\s*\[(\d{1,2}:\d{2})\s+(R|M)\]\s+(.+)$/i);
      if (!item || !item[3].trim() || !currentDate) continue;
      const speaker = item[2].toUpperCase(), content = item[3].trim();
      messages.push({ raw: line, date: currentDate, time: item[1], speaker, persona: speaker === "R" ? "river" : "mavc", content, normalized: normalize(content), tokens: tokens(content).map(stem) });
    }
  }
  return messages;
}
const CHAT_MEMORY = loadChatMemory();
console.log(`Memoria: ${CHAT_MEMORY.length} recuerdos cargados desde Anniversary.txt`);
...
function formatMemory(rows) { return rows.map((m, i) => `${i + 1}. [${m.date} ${m.time || ""}] ${m.speaker}: ${m.content}`).join("\n"); }
function randomMemoryWindow(size = 10) { if (!CHAT_MEMORY.length) return []; const maxStart = Math.max(0, CHAT_MEMORY.length - size); const start = Math.floor(Math.random() * (maxStart + 1)); return CHAT_MEMORY.slice(start, start + size); }
async function searchMemory(query) { const lexical = CHAT_MEMORY.map((m, i) => ({ i, score: lexicalScore(m, query) })).filter(x => x.score > 0).sort((a, b) => b.score - a.score).slice(0, 30); const semantic = await embeddingsService.semanticSearch(query); const candidates = new Map(); for (const x of semantic.slice(0, 8)) for (let i = x.chunk.start; i <= x.chunk.end; i++) candidates.set(i, (candidates.get(i) || 0) + x.score * 100); for (const x of lexical) candidates.set(x.i, (candidates.get(x.i) || 0) + x.score); const ranked = [...candidates.entries()].map(([i, score]) => ({ i, score })).sort((a, b) => b.score - a.score), selected = new Set(); for (const hit of ranked.slice(0, 10)) { const radius = hit.score >= 55 ? 4 : 2; for (let d = -radius; d <= radius; d++) { const i = hit.i + d; if (i >= 0 && i < CHAT_MEMORY.length) selected.add(i); } } const indexes = [...selected].sort((a, b) => a - b); for (const hit of ranked) { if (indexes.length >= MAX_MEMORY_MESSAGES) break; if (!selected.has(hit.i)) { indexes.push(hit.i); selected.add(hit.i); } } return { results: truncateMemory(indexes.slice(0, MAX_MEMORY_MESSAGES).map(i => CHAT_MEMORY[i])), confidence: Math.min(1, (ranked[0]?.score || 0) / 75), semantic: semantic.length > 0 }; }
